 ...
 * @author YvesFabienne
 */
public class ImageThreadDCRAW_9_12 extends Thread {
    ...
    public ImageThreadDCRAW_9_12(int image_width, int image_height, ShortPtr curveArg, boolean highlightArg, int finalOrientation, char[][][] image_sharp, int[] pixels, int nb_proc, int number) {
    ...
    void doLoop()  {
        ...
        for ( int x=xstart; x < xend; x++) {
            ...
            for( int y=ystart; y < yend; y++) {
                ...
                short bleu = CTOJ.toUnsigned((byte)(curve.at(Math.min( Math.max( 0, bleu16), 0xffff))>>8));
                short vert = CTOJ.toUnsigned((byte)(curve.at(Math.min( Math.max( 0, vert16), 0xffff))>>8));
                short rouge = CTOJ.toUnsigned((byte)(curve.at(Math.min( Math.max( 0, rouge16), 0xffff))>>8));
